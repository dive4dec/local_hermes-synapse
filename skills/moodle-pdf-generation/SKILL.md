...
- **dompdf SVG limitation:** dompdf cannot render MathJax SVG output (uses `<defs>`, `<use>`, `xlink:href`, `transform="scale(1,-1)"` — unsupported features). MathJax SVG approach was tested and reverted; Unicode conversion is the working approach.
